    ...
    userMove = (from: Key, to: Key) => {
        ...
        if (moveEquals(this.puzzle.moves[this.currentMove], move) === false) {
            ...
            this.ground.set({
                ...
                "lastMove": this.playedMoves.length > 0 ? chessgroundMove(this.playedMoves[this.playedMoves.length - 1]) : undefined,
            });
        }
        else {
            ...
            this.currentMove++;
            console.log(this.chess.turn, this.ground.state.movable.color, this.ground.state.turnColor);
            if (this.currentMove === this.puzzle.moves.length) {
                ...
                let cgMove = chessgroundMove(this.puzzle.moves[this.currentMove]);
                // ground.move doesn't seem to update the turnColor
                this.ground.set({"turnColor": opposite(this.chess.turn)});
                console.debug("auto playing next move, current move:", this.currentMove);
        ...
        this.currentPgn = this.playedMoves.map((move: NormalMove, i: number): string => makeSan(Chess.fromSetup(parseFen(this.fenHistory[i]).unwrap()).unwrap(), move)).join(" ");
    }
}