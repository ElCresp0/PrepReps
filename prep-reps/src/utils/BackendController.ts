...
export async function createUser(credentials: {name: string, password: string}): Promise<Response> {
    // curl -X POST localhost:3000/auth/sign_up -H "Content-Type: application/json" -d '{"name": "qbxtest", "password": "password"}'
    const response = await fetch(`http://${BACKEND_HOST}:${BACKEND_PORT}/auth/sign_up`, {
        method: "POST",
